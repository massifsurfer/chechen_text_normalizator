...
def normalize_ce(text, eliminate_nonchechen_diacritics=True):
    def remove_accents(input_str):
        ...
        cleaned = "".join([c for c in nfkd_form if ord(c) not in bad_diacritic_codes])
    ...
    wrong_palochka = "ιꞮꟾᶖÌ❘ÏĮïЇîǀĬ│ⅠїіӏᵻĨįıİᴉĪΙІΪÎĭɪΐ"
    standard_palochka = "Ӏ"
    text = re.sub(wrong_palochka, standard_palochka, text)

    for latin, cyrillic in letter_latin.items():
        text = re.sub(f"{latin}(?=[а-яА-ЯӀЁё])|(?<=[а-яА-ЯӀЁё]){latin}", cyrillic, text)
